...
test("Verify users can buy an item successfully", async ({
  ...
  await test.step("5. Verify the items should be displayed as a grid", async () => {
    ...
    expect
      .soft(await productPage.isGridView(), "Grid view should be active")
      .toBeTruthy();
  });
  ...
  await test.step("7. Verify the items should be displayed as a list", async () => {
    expect
      .soft(await productPage.isListView(), "List view should be active")
      .toBeTruthy();
  });
  ...
  await test.step("11. Verify item details in mini content", async () => {
    expect
      .soft(
        await shoppingCartPage.getMiniProductInfo(),
        "Mini cart product should match",
      )
      .toEqual(expectedProductInfo);
  });
  ...
  await test.step("14. Verify item details in order", async () => {
    expect
      .soft(
        await checkoutPage.getCheckoutProductInfo(),
        "Checkout product should match",
      )
      .toEqual(expectedProductInfo);
  });
  ...
  await test.step("17. Verify Order status page displays", async () => {
    expect
      .soft(
        await isOrderStatusPageDisplayed(),
  ...
  await test.step("18. Verify the Order details with billing and item information", async () => {
    expect
      .soft(
        await orderStatusPage.getOrderedProductInfo(),
        "Ordered product should match",
      )
      .toEqual(expectedProductInfo);
    expect
      .soft(await orderStatusPage.getBillingInfo(), "Billing info should match")
      .toEqual(expectedBilling);
  });
